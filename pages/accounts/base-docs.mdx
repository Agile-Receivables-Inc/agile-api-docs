...
2. **Push ledger entries** with the **Ledgers** endpoints (`POST /ledgers/bulk`). The server
   upserts each entry and immediately runs the statement aggregator — one monthly statement per
   `(customer, calendar month)` is created or updated, with payments allocated across statements
   oldest-first.
3. **Read back statements** with the **Statements** endpoints to confirm what was materialized.
4. **Enroll statements into a Statement-Acknowledgement campaign** with
